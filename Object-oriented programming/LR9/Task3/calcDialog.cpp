...
void InitBtnDescrArray()
{
    ...
    _btnDescr.push_back( BtnDescr("x^y", EXP) );
...
CalcDialog::CalcDialog( QWidget * parent)
{
    ...
    QVBoxLayout *Maksonishe = new QVBoxLayout();
    ...
    for (int i = 0; i < _btnDescr.size(); i++) {
        ...
        if(i<GRID_KEYS - 4) // Если кнопка из центрального блока - помещаем в сетку
            gridLayout->addWidget(button, i / 4, i % 4);
        else if (i < GRID_KEYS)
            Maksonishe->addWidget(button);
            ...
            mainKeysLayout->addLayout(Maksonishe);
